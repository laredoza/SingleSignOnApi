...
namespace SingleSignOn.Migrations.SqlServer
{
        ...
        where T : DbContext
    {
        ...
        public T CreateDbContext(string[] args)
        {
            ...
            optionsBuilder.UseMySql(connectionString, a => a.MigrationsAssembly(migrationsAssembly)); 
            services.AddTransient<DbContextOptions<ConfigurationDbContext>>( p => optionsBuilder.Options);

            services.AddDbContext<T>(options =>
            {
                options.UseMySql(connectionString, a => a.MigrationsAssembly(migrationsAssembly));
            });
